...
type FormatSqlArgs =
    ...
    interface IArgParserTemplate with
        member this.Usage =
            match this with
            | AuthToken _ -> "Accepted for SQL Prompt compatibility and ignored."
            | AdsStylesPath _ -> "Path to a directory of SQL Prompt-style formatting files."
            | StyleName _ -> "Style name to load from available style files. Defaults to 'Default'."
            | ApplyCasing -> "Accepted for SQL Prompt compatibility."

[<CliPrefix(CliPrefix.None)>]
type CliArgs =
    | [<CliPrefix(CliPrefix.None); CustomCommandLine("formatSql")>] FormatSql of ParseResults<FormatSqlArgs>
...
let private cliParser = ArgumentParser.Create<CliArgs>(programName = "tsqlfmt")

let formatSqlParser = cliParser.GetSubCommandParser <@ FormatSql @>
...
let parseArgs (argv: string[]) : Result<ResolvedArgs, string> =
    try
        ...
        | _ when argv.Length > 0 && argv.[0] = "formatSql" ->
            ...
            |> function
                | parsed when parsed.Contains <@ FormatSql @> ->
                    parsed.GetResult <@ FormatSql @> |> parseFormatSqlArgs |> Ok
        ...
        | _ -> Error "Missing SQL Prompt command: formatSql"
